...
export type SqlResult = { ok: true; schemaVersion: number; columns: { name: string; type: string | null }[]; rows: Record<string, unknown>[]; returnedRows: number; truncated: boolean; dataVersions: Record<string, string>; elapsedMs: number } | { ok: false; error: { code: string; message: string; hint?: string } };
...
export class DataWorkspace {
  ...
  write(input: WriteOptions): TableSchema {
    ...
    try {
      ...
      if (!existing && this.tables().some(d => d.name.toLowerCase() === input.table.toLowerCase())) throw new Error('Table names must not differ only by case');
      const primaryKey = input.primaryKey ?? existing?.primaryKey ?? [];
      if (existing && JSON.stringify(primaryKey) !== JSON.stringify(existing.primaryKey)) throw new Error('Primary key changes require a new table');
      ...
      for (const name of names) {
        ...
        if (previous && previous.type !== type) throw new Error(`Type change for ${name}; create a new table`);
  ...
  relate(relation: Relation) {
    for (const end of [relation.from, relation.to]) if (!this.schema(end.table).columns[end.column]) throw new Error(`Unknown column: ${end.table}.${end.column}`);
  ...
  async sql(sql: string, params: (string | number | null)[] = [], options: SqlOptions = {}): Promise<SqlResult> {
    ...
    return new Promise(resolveResult => {
      ...
      let finished = false;
      const finish = (result: SqlResult) => { if (finished) return; finished = true; clearTimeout(timer); worker.kill('SIGKILL'); resolveResult(result); };
  ...
  tools(): AgentTool[] {
    ...
    return [
      ...
      tool('schema', 'Inspect schema, grain, units, scope and relations.', { table: { type: 'string' } }, [], i => i.table === undefined ? this.tables().map(t => this.schema(t.name)) : this.schema(i.table)),
